...
class LossCanvas(FigureCanvas):
    ...
    def plot_loss_curve(self, iterations, train_losses, test_losses=None):
        self.axes.clear()
        self.axes.plot(iterations, train_losses, "b-", linewidth=2, label="训练损失")
        if test_losses is not None and len(test_losses) > 0:
            # 创建相同长度的x轴值
            test_x = np.linspace(iterations[0], iterations[-1], len(test_losses))
            self.axes.plot(test_x, test_losses, "r-", linewidth=2, label="测试损失")
...
# Simulate BP network training loss
def train_bp_network(X_tensor, y_tensor, hidden_size, penalty_factor, has_constraints, train_size=None, test_size=None, epochs=100, batch_size=8, lr=0.01):
    """训练BP神经网络模型并返回训练过程数据
    ...
    output_size = 1
    model = BPModel(input_size, hidden_size, output_size, device=device)
    model.to(device)
...
class BPPage(QWidget):
    ...
    def train_network(self):
        ...
        try:
            ...
            # 配置训练参数
            epochs = n + 10  # 使用n参数设置训练周期数
            batch_size = max(2, min(8, m // 2))  # 使用m参数影响批次大小
            
            # 执行训练
            # 如果数据量较少，调整批次大小
            if len(self.X_tensor) < 10:
                batch_size = 1
            elif len(self.X_tensor) < 20:
                batch_size = 2
                
            self.model, iterations, train_losses, test_losses, early_stop_message = train_bp_network(
                self.X_tensor, self.y_tensor, 
                hidden_size, penalty_factor, has_constraints,
                train_size=m, test_size=n,
    ...
    def load_model(self):
        ...
        try:
            ...
            if file_name:
                ...
                self.model = BPModel(input_size, hidden_size, output_size, device=device)
