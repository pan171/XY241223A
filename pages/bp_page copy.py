...
def simulate_bp_training(
    ...
    if has_constraints:
        ...
        noise_factor = 0.05 * (1 + penalty_factor / 20)
    else:
        noise_factor = 0.02
    ...
    # Add noise and oscillations to make it look realistic
    noise = np.random.normal(0, noise_factor, num_iterations) * base_loss
    oscillation = 0.1 * np.sin(iterations / 5) * base_loss

    loss_values = base_loss + noise + oscillation
...
class BPPage(QWidget):
    ...
    def train_network(self):
        ...
        try:
            ...
            iterations, loss_values = simulate_bp_training(
            ...
            QMessageBox.information(
                self, "Training Complete", "BP network training completed successfully!"
            )
